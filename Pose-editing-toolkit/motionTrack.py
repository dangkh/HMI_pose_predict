...
def savekeyframevideo(obj, list, width, height,filename):
    tmp = []
    ...
    out = cv2.VideoWriter(rawname+'keyframe.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (frame_width,frame_height))
    stater_frame = 0
    while(True):
        frame = obj[list[stater_frame]]
        # print(stater_frame)
        stater_frame += 1
        if stater_frame >= len(list):
            break
...
def runMotionTrack(filename):
    ...
    n=len(obj)
    unbreakable = True 
    if n>0 and unbreakable:
        J=indicekeyframe(obj,20)
        i=chooseframe(obj,fps,J,height,False)
        savekeyframevideo(obj,J, width, height,filename)
        unbreakable = False
